...
if [ -d "kernel_phoenix" ]; then
    ...
    git clone https://github.com/shripal17/android_kernel_xiaomi_sm6150 -b 15-pure-ksu kernel_phoenix
    echo -e "$green Kernel repository cloned successfully. $white"
fi


# Initialize Kernel
echo -e "$green Checking for KernelSU directory... $white"
if [ -d "kernel_phoenix/drivers/kernelsu" ]; then
    echo -e "$green KernelSU directory already exists. Skipping setup. $white"
else
    echo -e "$green Setting up KernelSU... $white"
    curl -LSs "https://raw.githubusercontent.com/tiann/KernelSU/main/kernel/setup.sh" | bash -s v0.9.5
    echo -e "$green KernelSU setup successfully. $white"
fi
...
export zipname="Pure-phoenix-KSU-${date}.zip"
...
echo "Kernel compilation started for device 'Sweet'."
